...
class TestUIWorkflowStateTransitions:
    ...
    @patch('LogonDetails.save_options')
    def test_options_saved_after_collection(self, mock_save):
        """Test that options are saved after collection"""
        with patch('LogonDetails.load_previous_options', return_value={}):
            with patch('LogonDetails.ask_string', side_effect=["User", "US", "123456", "5", "pass", ""]):
                with patch('LogonDetails.ask_rating', return_value="0"):
                    with patch('LogonDetails.ask_yesno', return_value=False):
                        with patch('LogonDetails.tk.Tk'):
                            with patch('LogonDetails.tk._default_root', None):
                                collect_user_input()
        
        # Verify save was called
        mock_save.assert_called_once()
        # Verify the saved options contain required fields
        saved_options = mock_save.call_args[0][0]
        for field in BASIC_FIELDS:
            assert field in saved_options
...
class TestConfigurationValidation:
    ...
    def test_all_basic_fields_have_defaults(self):
        ...
        for field in BASIC_FIELDS:
            assert field in DEFAULT_FIELDS, f"Field '{field}' missing from DEFAULT_FIELDS"
    ...
    def test_required_fields_not_empty_defaults(self, field):
        ...
        # This is a workflow test - actual filling happens in collect_user_input
    ...
    def test_yes_no_fields_have_valid_defaults(self):
        ...
        for field in yn_fields:
            if field in DEFAULT_FIELDS:
                assert DEFAULT_FIELDS[field] in ["y", "n"], \
    ...
    def test_choice_fields_have_valid_defaults(self):
        ...
        choice_fields = {"coast_choice": ["1", "2", "3"], "land_choice": ["1", "2", "3"]}
        for field, valid_options in choice_fields.items():
            if field in DEFAULT_FIELDS:
                assert DEFAULT_FIELDS[field] in valid_options, \
